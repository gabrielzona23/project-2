...
# Bateria 2 - Escalabilidade Progressiva (Docker K6 Version)
# Data: 01/10/2025
...
K6_SCRIPT="benchmark/k6-bateria-2-escalabilidade-fixed.js"
...
for runtime in "${runtimes[@]}"; do
    ...
    if curl -s "http://localhost:${port}/api/static" | grep -q "status"; then
...
# Execute K6 test using Docker
echo -e "${BLUE}🐳 Executando K6 via Docker...${NC}"
docker run --rm \
    --network host \
    -v $(pwd):/app \
    -w /app \
    grafana/k6:latest run \
    --out json="/app/$RESULTS_DIR/k6_results.json" \
    --summary-export="/app/$RESULTS_DIR/k6_summary.json" \
